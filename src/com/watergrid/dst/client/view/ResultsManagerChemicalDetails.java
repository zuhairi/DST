...
public class ResultsManagerChemicalDetails extends Composite{
	...
	//for nodes data grid
	Label lblNodeTitle;
	DataGrid<NodeChemicalResult> nodeDataGrid;
	SimplePager nodePager;
	ListDataProvider<NodeChemicalResult> nodeDataProvider;
	SingleSelectionModel<NodeChemicalResult> nodeSelectionModel;
	List<NodeChemicalResult> nodeList;
	...
	public ResultsManagerChemicalDetails(ResultsManager parentView){
	...
	private void initializeComponents() {
		...
		lblNodeTitle = new Label("NODE - Chemical Concentration (mg/L) ");
		...
		btnBack.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				parentView.activateChemicalSummaryMode();
			}
		});
		...
		cmbVariable.addItem("chemicalCon");
	...
	private void createNodesDataGrid() {
		...
		nodeDataGrid = new DataGrid<NodeChemicalResult>();
		...
		// Create Name column.
		TextColumn<NodeChemicalResult> idColumn = new TextColumn<NodeChemicalResult>() {
			@Override
			public String getValue(NodeChemicalResult nodeResult) {
				return nodeResult.getNodeId();
			}
		};
		// Create MinChemical column.
		TextColumn<NodeChemicalResult> minChemical = new TextColumn<NodeChemicalResult>() {
			@Override
			public String getValue(NodeChemicalResult nodeResult) {
				return NumberFormat.getFormat("0.00").format(nodeResult.getMinChemical());
			}
		};
		// Create MaxChemical column.
		TextColumn<NodeChemicalResult> maxChemical = new TextColumn<NodeChemicalResult>() {
			@Override
			public String getValue(NodeChemicalResult nodeResult) {
				return NumberFormat.getFormat("0.00").format(nodeResult.getMaxChemical());
			}
		};
		// Create AvgChemical column.
		TextColumn<NodeChemicalResult> avgChemical = new TextColumn<NodeChemicalResult>() {
			@Override
			public String getValue(NodeChemicalResult nodeResult) {
				return NumberFormat.getFormat("0.00").format(nodeResult.getAvgChemical());
			}
		};
		...
		nodeDataGrid.addColumn(minChemical, "min");
		nodeDataGrid.addColumn(maxChemical, "max");
		nodeDataGrid.addColumn(avgChemical, "avg");

		// Create a data provider.
		nodeDataProvider = new ListDataProvider<NodeChemicalResult>();
		...
		// selection Model
		nodeSelectionModel = new SingleSelectionModel<NodeChemicalResult>();
		nodeDataGrid.setSelectionModel(nodeSelectionModel);
		
		//Sorters for the dataGrid
		
		idColumn.setSortable(true);
		minChemical.setSortable(true);
		maxChemical.setSortable(true);
		avgChemical.setSortable(true);
		
		ListHandler<NodeChemicalResult> columnSortHandler = new ListHandler<NodeChemicalResult>(nodeDataProvider.getList());
		nodeDataGrid.addColumnSortHandler(columnSortHandler);
		
		columnSortHandler.setComparator(idColumn, ComparatorFactory.getComp_NodeChemical_Id());
		columnSortHandler.setComparator(minChemical, ComparatorFactory.getComp_NodeChemical_MinChemicalCon());
		columnSortHandler.setComparator(maxChemical, ComparatorFactory.getComp_NodeChemical_MaxChemicalCon());
		columnSortHandler.setComparator(avgChemical, ComparatorFactory.getComp_NodeChemical_AvgChemicalCon());
	...
	private void viewOnMapButtonPressed(){
		...
		NodeChemicalResult selectedNode = nodeSelectionModel.getSelectedObject();
...
private void viewGraphButtonPressed(){
	...
	public void updateNodesData(List<NodeChemicalResult> data){
	...
	private void loadGraph(ArrayList<Float> data,String variable,String componentId){
		...
		if(variable.equals("chemicalCon")){
			vTitle="Chemical Concentration (mg/L)";
		}
