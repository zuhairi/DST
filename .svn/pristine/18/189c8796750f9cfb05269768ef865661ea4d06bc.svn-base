...
public class ResultsManagerHydraulicSummary extends Composite{
	...
	//labels to add data
	Label lblMinP;		//pressure
	Label lblMaxP;
	Label lblMinH;		//head
	Label lblMaxH;
	Label lblMinF;		//flow
	Label lblMaxF;
	Label lblMinV;		//velocity
	Label lblMaxV;
	Label lblMinHL;		//headLoss
	Label lblMaxHL;
	...
	public ResultsManagerHydraulicSummary(ResultsManager parentView){
	...
	private void initializeComponents(){
		...
		Image imgLogo = new Image(images.gaugeBig());
		verticalPanelGlobal.add(imgLogo);
		
		Label lblTitle = new Label("Hydraulic Results Summary");
		...
		Label lblPressureTitle = new Label("Pressure (m): ");
		grid.setWidget(1, 0, lblPressureTitle);
		
		Label lblHeadTitle = new Label("Head (m): ");
		grid.setWidget(2, 0, lblHeadTitle);
		
		Label lblFlowTitle = new Label("Flow (litres/s): ");
		grid.setWidget(3, 0, lblFlowTitle);
		
		Label lblVelocityTitle = new Label("Velocity (m/s): ");
		grid.setWidget(4, 0, lblVelocityTitle);
		
		Label lblHeadLossTitle = new Label("HeadLoss (m/km): ");
		grid.setWidget(5, 0, lblHeadLossTitle);
		
		lblMinP = new Label("0.00");
		grid.setWidget(1, 2, lblMinP);
		lblMaxP = new Label("0.00");
		grid.setWidget(1, 4, lblMaxP);
		
		lblMinH = new Label("0.00");
		grid.setWidget(2, 2, lblMinH);
		lblMaxH = new Label("0.00");
		grid.setWidget(2, 4, lblMaxH);
		
		lblMinF = new Label("0.00");
		grid.setWidget(3, 2, lblMinF);
		lblMaxF = new Label("0.00");
		grid.setWidget(3, 4, lblMaxF);
		
		lblMinV = new Label("0.00");
		grid.setWidget(4, 2, lblMinV);
		lblMaxV = new Label("0.00");
		grid.setWidget(4, 4, lblMaxV);
		
		lblMinHL = new Label("0.00");
		grid.setWidget(5, 2, lblMinHL);
		lblMaxHL = new Label("0.00");
		grid.setWidget(5, 4, lblMaxHL);
		
		
		Label lblAlertsTitle = new Label("Alerts: ");
		grid.setWidget(7, 0, lblAlertsTitle);
		
		lblAlerts = new Label("0");
		grid.setWidget(7, 2, lblAlerts);
		...
		btnComponentDetails.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				parentView.showHydraulicDetails();
			}
		});
	...
	public void loadSummaryData(ArrayList<Float> summaryData){
		lblMinP.setText(String.valueOf(summaryData.get(0)));		
		lblMaxP.setText(String.valueOf(summaryData.get(1)));
		lblMinH.setText(String.valueOf(summaryData.get(2)));	
		lblMaxH.setText(String.valueOf(summaryData.get(3)));
		lblMinF.setText(String.valueOf(summaryData.get(4)));
		lblMaxF.setText(String.valueOf(summaryData.get(5)));
		lblMinV.setText(String.valueOf(summaryData.get(6)));
		lblMaxV.setText(String.valueOf(summaryData.get(7)));
		lblMinHL.setText(String.valueOf(summaryData.get(8)));
		lblMaxHL.setText(String.valueOf(summaryData.get(9)));
	}
	...
	public void loadAlertsData(int alertCount){
		...
		if (alertCount==0){
			...
			grid.setWidget(7, 4, alertStatus);
		}else{
