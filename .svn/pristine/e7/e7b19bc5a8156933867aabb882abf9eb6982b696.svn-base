...
public class ResultsManagerAgeDetails extends Composite{
	...
	//for nodes data grid
	Label lblNodeTitle;
	DataGrid<NodeAgeResult> nodeDataGrid;
	SimplePager nodePager;
	ListDataProvider<NodeAgeResult> nodeDataProvider;
	SingleSelectionModel<NodeAgeResult> nodeSelectionModel;
	List<NodeAgeResult> nodeList;
	...
	public ResultsManagerAgeDetails(ResultsManager parentView){
	...
	private void initializeComponents() {
		...
		lblNodeTitle = new Label("NODE - Age (hours) ");
		...
		btnBack.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				parentView.activateAgeSummaryMode();
			}
		});
	...
	private void createNodesDataGrid() {
		...
		nodeDataGrid = new DataGrid<NodeAgeResult>();
		...
		// Create Name column.
		TextColumn<NodeAgeResult> idColumn = new TextColumn<NodeAgeResult>() {
			@Override
			public String getValue(NodeAgeResult nodeResult) {
				return nodeResult.getNodeId();
			}
		};
		// Create MinAge column.
		TextColumn<NodeAgeResult> minAge = new TextColumn<NodeAgeResult>() {
			@Override
			public String getValue(NodeAgeResult nodeResult) {
				return NumberFormat.getFormat("0.00").format(nodeResult.getMinAge());
			}
		};
		// Create MaxAge column.
		TextColumn<NodeAgeResult> maxAge = new TextColumn<NodeAgeResult>() {
			@Override
			public String getValue(NodeAgeResult nodeResult) {
				return NumberFormat.getFormat("0.00").format(nodeResult.getMaxAge());
			}
		};
		// Create AvgAge column.
		TextColumn<NodeAgeResult> avgAge = new TextColumn<NodeAgeResult>() {
			@Override
			public String getValue(NodeAgeResult nodeResult) {
				return NumberFormat.getFormat("0.00").format(nodeResult.getAvgAge());
			}
		};
		
		// Add the columns.
		nodeDataGrid.addColumn(idColumn, "ID");
		nodeDataGrid.addColumn(minAge, "min");
		nodeDataGrid.addColumn(maxAge, "max");
		nodeDataGrid.addColumn(avgAge, "avg");

		// Create a data provider.
		nodeDataProvider = new ListDataProvider<NodeAgeResult>();
		...
		// selection Model
		nodeSelectionModel = new SingleSelectionModel<NodeAgeResult>();
		nodeDataGrid.setSelectionModel(nodeSelectionModel);
		
		//Sorters for the dataGrid
		
		idColumn.setSortable(true);
		minAge.setSortable(true);
		maxAge.setSortable(true);
		avgAge.setSortable(true);
		
		ListHandler<NodeAgeResult> columnSortHandler = new ListHandler<NodeAgeResult>(nodeDataProvider.getList());
		nodeDataGrid.addColumnSortHandler(columnSortHandler);
		
		columnSortHandler.setComparator(idColumn, ComparatorFactory.getComp_NodeAge_Id());
		columnSortHandler.setComparator(minAge, ComparatorFactory.getComp_NodeAge_MinAge());
		columnSortHandler.setComparator(maxAge, ComparatorFactory.getComp_NodeAge_MaxAge());
		columnSortHandler.setComparator(avgAge, ComparatorFactory.getComp_NodeAge_AvgAge());
	...
	private void viewOnMapButtonPressed(){
		...
		NodeAgeResult selectedNode = nodeSelectionModel.getSelectedObject();
	...
	public void updateNodesData(List<NodeAgeResult> data){
